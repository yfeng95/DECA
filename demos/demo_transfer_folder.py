...
def main(args):
    ...
    for i in range(len(expdata)):
        ...
        transfer_opdict, transfer_visdict = deca.decode(id_codedict, id_dict)
        id_visdict['transferred_shape'] = transfer_visdict['shape_detail_images']
        cv2.imwrite(os.path.join(savefolder, name + f'_animation_{exp_name}.jpg'), deca.visualize(id_visdict))
        ...
        if args.saveDepth or args.saveKpt or args.saveObj or args.saveMat or args.saveImages:
            ...
            os.makedirs(os.path.join(savefolder, name, f'animation_{exp_name}'), exist_ok=True)
        
        # -- save results
        image_name = name
        for save_type in ['reconstruction', 'animation']:
            # name = f'{image_name}_{exp_name}'
            if save_type == 'reconstruction':
                visdict = id_codedict; opdict = id_opdict
            else:
                save_type = f'animation_{exp_name}'
            ...
            if args.saveObj:
                deca.save_obj(os.path.join(savefolder, name, save_type, name + exp_name + '.obj'), opdict)
            if args.saveMat:
